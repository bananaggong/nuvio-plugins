...
이 저장소에는 서비스 소스 코드나 데이터베이스 자격 증명이 포함되지 않습니다. `NUVIO Host Center` plugin은 사전 등록된 NUVIO Remote MCP connection을 참조하고, 사용자는 NUVIO 로그인 후 자신이 접근할 수 있는 호스트센터와 read-only 권한만 승인합니다.
